...
async function rpcOwnerOf(tokenId) {
  const body = {
    ...
    id: 1,
    method: "eth_call",
    params: [{ to: CONTRACT, data: encodeOwnerOfCall(tokenId) }, "latest"],
  };
...
async function verifyOwnershipOnChain(wallet, candidateIds) {
  ...
  for (let i = 0; i < candidateIds.length; i += batchSize) {
    ...
    const owners = await Promise.all(chunk.map((id) => rpcOwnerOf(id).catch(() => null)));
...
async function fetchWalletOtterIds(wallet) {
  ...
  return {
    ...
    hint:
      tokenIds.length === 0 ? "No Otterful Otters found for this wallet on ApeChain." : undefined,
  };
}

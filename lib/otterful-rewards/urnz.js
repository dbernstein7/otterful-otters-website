 ...
 *
 * API contract sourced from the Otterful Rewards Discord bot (urnzClient.js):
 * - Base URL: https://www.urnz.app/api/v1 (override via URNZ_BASE_URL)
 * - Auth: Authorization: Bearer {URNZ_API_KEY} (also accepts x-urnz-key per URNZ gateway)
 * - Idempotency: Idempotency-Key request header
 * - GET  /realms/{realmId}/currencies
 * - GET  /realms/{realmId}/members/search?type={type}&values={value}
 * - PATCH /realms/{realmId}/members/{memberId}/balance  body: { amount, currencyId, action }
 *
 * Wallet member lookup uses type=wallet (Drip-compatible search; verify against your realm).
 */
...
function getUrnzConfig(env = process.env) {
  ...
  const realmId = String(env.URNZ_REALM_ID || "").trim();
  const baseUrl = String(env.URNZ_BASE_URL || "https://www.urnz.app/api/v1")
    .trim()
    .replace(/\/$/, "");
  const memberSearchType = String(env.URNZ_MEMBER_SEARCH_TYPE || "wallet").trim() || "wallet";
...
async function findMemberByWallet(config, wallet) {
  ...
  if (!w) {
    return { ok: false, code: "bad_wallet", message: "Wallet required for URNZ member lookup." };
  }

  const attempts = [{ type: config.memberSearchType, values: w }];
  const lower = w.toLowerCase();
  if (lower !== w && config.memberSearchType === "wallet") {
    attempts.push({ type: "wallet", values: lower });
  }

  for (const attempt of attempts) {
    const res = await searchMembers(config, attempt.type, attempt.values);
    if (!res.ok) {
      return { ok: false, code: res.code, message: res.message, status: res.status };
    }
    const members = res.data?.data || [];
    if (Array.isArray(members) && members.length > 0 && members[0]?.id) {
...
function resolveMemberId(member) {
  const candidates = [member?.realmMemberId, member?.id].filter(
...
async function award(wallet, amount, transaction = {}, opts = {}) {
  ...
  if (!memberId) {
    return { ok: false, code: "bad_member", message: "URNZ member id missing or invalid." };
  }
  ...
  const responseBalance =
    typeof res.data?.balance === "number"
      ? res.data.balance
      : memberBalanceForCurrency(found.member, currency.id) + points;
  ...
  return {
    ...
    currencyId: currency.id,
    externalReference: res.data?.id || res.data?.transactionId || memberId,
  };
}

...
async function bakeWearableGlbForMml(buffer, kind) {
  ...
  for (const node of parts) {
    ...
    mat4.multiply(rel, socketInv, nodeWorld);
    transformMeshVertices(node.getMesh(), rel);
    node.setMatrix(IDENTITY);
  }

  if (kind === 'shirt') {
    const lift = parseFloat(process.env.MML_SHIRT_BAKE_Y || '0.28');
    if (Number.isFinite(lift) && lift !== 0) {
      const liftMat = mat4.fromTranslation(mat4.create(), [0, lift, 0]);
      for (const node of parts) {
        transformMeshVertices(node.getMesh(), liftMat);
      }
    }
  }

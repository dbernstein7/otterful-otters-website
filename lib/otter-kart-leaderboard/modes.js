...
function memberDisplayShort(member, meta) {
  if (isDemoMember(member) || meta?.isDemo) {
    const raw = meta?.demoShort || String(member || "").replace(/^demo:/, "");
    return `Demo ${shortWallet(raw)}`;
  }
...
function buildMeta(mode, wallet, stats, opts = {}) {
  if (opts.isDemo) {
    const member = demoMemberKey(wallet);
    const demoShort = shortWallet(String(wallet || ""));
    return {
      wallet: member || `demo:${String(wallet || "").slice(0, 64)}`,
      walletShort: `Demo ${demoShort}`,
      isDemo: true,
      demoShort,
      mode,

 ...
 */
async function upsertSessionLeaderboard(wallet, points, shells) {
  if (!isKvConfigured()) return { updated: false, reason: "not_configured" };
  const w = String(wallet || "").trim().toLowerCase();
  if (!/^0x[a-f0-9]{40}$/.test(w)) return { updated: false, reason: "bad_wallet" };

  const pts = Math.max(0, Math.floor(Number(points) || 0));
  const sh = Math.max(0, Math.floor(Number(shells) || 0));
  if (pts <= 0 && sh <= 0) return { updated: false, reason: "empty" };

  const score = pts * 1e6 + sh;
  const prev = await zscore(SESSION_POINTS_KEY, w);
  if (!isBetterScore("session", score, prev)) {
    return { updated: false, reason: "not_improved" };
  }

  await zadd(SESSION_POINTS_KEY, score, w);
  if (sh > 0) await zadd(SESSION_SHELLS_KEY, sh, w);
  await hset(
    SESSION_META_KEY,
    w,
    JSON.stringify({
...
async function upsertLeaderboardEntry(wallet, mode, stats) {
  if (!isKvConfigured()) return { updated: false, reason: "not_configured" };
  if (mode === "session") {
    return upsertSessionLeaderboard(wallet, stats.points, stats.shells);
  }
  ...
  await hset(metaKey, member, JSON.stringify(buildMeta(mode, wallet, { ...stats, dateISO })));
  return { updated: true };
}
